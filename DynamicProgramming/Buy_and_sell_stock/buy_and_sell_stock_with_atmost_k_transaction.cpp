...
int solve(int index, bool canBuy, vector<int>& prices, int num_transaction) 
{
    ...
    if (canBuy) {
        ...
        int buy = -prices[index] + solve(index + 1, false, prices,num_transaction);
...
int solve_tab(vector<int>& prices, int k)
{
    ...
    for(int index=n-1; index>=0; index--)
    {
        ...
        {
            for(int num_transaction=1; num_transaction<=k; num_transaction++)
            {
                int profit=0;
                if (canBuy) {
                    // Two options: Buy or skip
...
int solve_spaceoptim(vector<int>& prices, int k)
{
    int n = prices.size();
...
int main()
{
  ...
  int k=2;
  cout<<solve_spaceoptim(prices,k);
  return 0;
}
